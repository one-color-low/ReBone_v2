...
sandbox_sess = tf.Session(config=tf.ConfigProto(device_count=device_count, allow_soft_placement=True))
#plt.figure(3)
batch_size = 128
sandbox_model = create_model(sandbox_sess, actions, batch_size)
...
def baseline(json_path):
    ...
    for n, (frame, xy) in enumerate(smoothed.items()):
        ...
        _, _, poses3d = sandbox_model.step(sandbox_sess, enc_in, dec_out, dp, isTraining=False)
        all_poses_3d = []

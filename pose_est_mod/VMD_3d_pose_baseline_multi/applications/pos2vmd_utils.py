...
def load_slope_vmd(is_upper2_body):
    ...
    if is_upper2_body:
        # 上半身2がある場合
        filename = join(dirname(abspath(dirname(__file__))),"slope/slope_upper2.vmd")
        return reader.read_vmd_file(filename)
    else:
        # 標準ボーンのみの場合
        filename = join(dirname(abspath(dirname(__file__))),"slope/slope_normal.vmd")
        return reader.read_vmd_file(filename)
